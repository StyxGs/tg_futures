...
def create_tracked_list(list_futures: list) -> InlineKeyboardBuilder:
    ...
    for future in list_futures:
        futures_buttons.append(InlineKeyboardButton(text=future[0], callback_data=f'{future[0]}*'))
...
def delete_future(list_futures: list) -> InlineKeyboardBuilder:
    """Создание инлайн-кнопки для удаления отслеживаемых фьючерсов"""
    futures_del_kb: InlineKeyboardBuilder = InlineKeyboardBuilder()
    futures_buttons = []
    for future in list_futures:
        futures_buttons.append(InlineKeyboardButton(text=f'{LEXICON_RU["del"]} {future[0]}',
                                                    callback_data=f'{future[0]}del'))
...
def list_of_available_futures() -> InlineKeyboardBuilder:
